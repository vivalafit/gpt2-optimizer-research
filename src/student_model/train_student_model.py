...
def train_student_model():
    ...
    print("Initializing StudentGPT2Model")
    model = StudentGPT2Model(config).to(device)

    print("Applying pruning to the model")
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            prune.l1_unstructured(module, name='weight', amount=0.4)  # Применяем L1 прунинг к линейным слоям
